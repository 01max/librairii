...
WebView2 runtime. CI checks Microsoft's machine and current-user WebView2
registry keys before acceptance. When neither contains a usable version, it
downloads the official Evergreen bootstrapper, verifies its Microsoft
signature, installs it silently, and requires the runtime to appear in the
registry before continuing. The gate installs Librairii into an isolated
user-owned directory, validates its current-user uninstall registration and
both shortcuts, and runs acceptance from the installed copy. It then
